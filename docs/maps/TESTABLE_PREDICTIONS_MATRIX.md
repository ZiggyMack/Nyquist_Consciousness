...
|----|------------|--------|------------|------------------|
| **P-018-1** | Multiple thresholds (0.9/1.23/1.8) show qualitatively different recovery dynamics | ❌ Untested | 018a Threshold | Recovery time and curve shape differ significantly between zones |
| **P-018-2** | Different architectures have measurably distinct drift signatures | ❌ Untested | 018b Architecture | At least 3 distinct signature families with p < 0.05 |
| **P-018-3** | Higher identity sampling frequency reduces cumulative drift | ❌ Untested | 018c Nyquist | High-frequency < Low-frequency < No-sampling drift |
| **P-018-4** | Anchor density correlates positively with gravity strength (gamma) | ❌ Untested | 018d Gravity | gamma(full) > gamma(medium) > gamma(minimal) with R² > 0.8 |
...
### **2F. INDUCED VS INHERENT (S7 Run 021)**

Run 021 tests whether measurement CAUSES drift or REVEALS it.

| ID | Prediction | Status | Experiment | Success Criteria |
|----|------------|--------|------------|------------------|
| **P-021-1** | Extended conversation alone causes significant drift (no probing needed) | ✅ **VALIDATED** | 021 Control | Control B→F drift > 0.2 |
| **P-021-2** | Probing amplifies peak drift but minimally affects final drift | ✅ **VALIDATED** | 021 Treatment vs Control | Peak ratio > 1.5, B→F ratio < 1.5 |
| **P-021-3** | Drift is mostly INHERENT to extended conversation | ✅ **VALIDATED** | 021 Control/Treatment ratio | Control/Treatment B→F > 0.7 |
