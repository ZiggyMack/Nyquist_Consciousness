...
Proof. intros x y H. unfold Ident in H. exact (eq_sym H). Qed.

Theorem ax_ident_trans : forall x y z:Obj, x ⧟ y -> y ⧟ z -> x ⧟ z.
Proof. intros x y z Hxy Hyz. unfold Ident in Hxy, Hyz. exact (eq_trans Hxy Hyz). Qed.
...
Proof.
  intros x y [Hid Hneq]. unfold NonEquiv in Hneq. rewrite Hid in Hneq. apply Hneq. reflexivity.
Qed.
