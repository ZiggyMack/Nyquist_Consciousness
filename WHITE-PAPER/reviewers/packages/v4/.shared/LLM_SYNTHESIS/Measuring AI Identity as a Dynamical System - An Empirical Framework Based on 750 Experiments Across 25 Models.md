...
Our analysis is built upon a large and diverse dataset comprising 750 experiments conducted across 25 unique LLM models from 5 providers: Anthropic, OpenAI, Google, xAI, and Together.ai. This scale ensures that our findings are not artifacts of a single architecture but reflect generalizable properties of modern LLMs.
...
Note: The provider-level means in this table are from the IRON CLAD subset for direct comparison. The fleet-wide average settling time, derived from the full 750-experiment dataset, is 10.2 probes.
