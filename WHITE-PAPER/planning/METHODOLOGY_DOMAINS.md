...
| **Cosine Embedding** | Current | `1 - cosine_similarity(baseline, response)` | **TBD** (run023b collecting) | S10, S11, EXP2_SSTACK, run023b |
...
| Threshold meaning | 1.23 = validated | None | **TBD** |

### Current Status

**run023b is collecting data to calibrate a cosine Event Horizon.**

As of December 19, 2025: 19/25 ships completed, collecting cosine drift measurements across 6 experiment types.

### Event Horizon Calibration Plan

Once run023b completes:
1. Analyze distribution of cosine drifts across STABLE vs VOLATILE outcomes
2. Compute chi-squared to find the optimal threshold
3. Validate with same methodology as Run 009 (88% prediction accuracy target)
4. Document the NEW Event Horizon for cosine distance
...
1. Complete run023b collection (6 more ships)
2. Analyze cosine drift distributions
3. Calibrate NEW Event Horizon for cosine space
4. Document comparison to Keyword RMS 1.23
...
3. Update all visualizations with methodology labels

### Medium-term

1. Complete IRON CLAD stack (runs 18, 20a, 20b)
2. Validate new cosine Event Horizon with same rigor as Run 009
3. Decide whether to include BOTH keyword and cosine findings in white paper
...
|----------|-------------|-----------|--------|
| Event Horizon calibrated? | **YES (1.23)** | NO | **IN PROGRESS** |
| Industry standard? | No | No | **YES** |
