...
WORKSHOP_CONTENT = PathContent(
    ...
    blueprints=[],  # Blueprints are internal-only, not pushed to reviewers
    # All shared content (theory, guides, figures, references, planning) lives in .shared/
    # Path packages only contain path-specific submissions
    theory=[],
    guides=[],
    references=[],
    figures=[],
...
def extract_submission_pdfs(output_dir: Optional[Path] = None, dry_run: bool = False) -> Dict:
    """
    Extract all submission PDFs to a flat folder structure for easy distribution.

    Copies from: WHITE-PAPER/submissions/{path}/*.pdf
    Copies to:   reviewers/packages/pdf/{FILENAME}.pdf
    ...
    # Collect all PDFs from submissions/{path}/ directories
    collected_pdfs: List[Path] = []
    by_path: Dict[str, List[Path]] = {}

    for path_name in PUBLICATION_PATHS:
        path_dir = submissions_dir / path_name
        if path_dir.exists():
            # Use set to avoid duplicates (Windows is case-insensitive)
            pdfs = set(path_dir.glob("*.pdf")) | set(path_dir.glob("*.PDF"))
            if pdfs:
                by_path[path_name] = sorted(pdfs)
                collected_pdfs.extend(pdfs)

    total_files = len(collected_pdfs)
    total_size = calculate_total_size(collected_pdfs)

    result = {
        "mode": "submission_pdfs",
        "output_dir": str(output_dir),
        "dry_run": dry_run,
        "statistics": {
            "total_files": total_files,
    ...
    for path_name, files in by_path.items():
        result["by_path"][path_name] = {
            "count": len(files),
            "files": [f.name for f in files]
        }

    if dry_run:
        result["message"] = f"Would copy {total_files} submission PDFs ({format_size(total_size)}) to {output_dir}"
        return result
    ...
    # Copy all PDFs to flat structure
    for src_pdf in collected_pdfs:
        dst_pdf = output_dir / src_pdf.name
        try:
            shutil.copy2(src_pdf, dst_pdf)
            result["files_copied"].append(src_pdf.name)
        except Exception as e:
            result["errors"].append(f"Failed to copy {src_pdf.name}: {e}")

    # Generate README for the submission_pdfs directory
    readme_lines = [
        "# Submission PDFs",
        "",
        f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    for path_name, info in result["by_path"].items():
        readme_lines.append(f"| {path_name} | {', '.join(info['files'])} |")
    ...
    for path_name, files in by_path.items():
        for f in files:
            readme_lines.append(f"| `{f.name}` | `submissions/{path_name}/{f.name}` |")
    ...
    result["message"] = f"Copied {total_files} submission PDFs ({format_size(total_size)}) to {output_dir}"

    return result


def generate_submission_pdf_report(result: Dict) -> str:
    """Generate a report of submission PDF extraction results."""

    lines = [
        "=" * 70,
        "SUBMISSION PDF EXTRACTION REPORT",
        f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "=" * 70,
        "",
    ]
    ...
    for path_name, info in result.get("by_path", {}).items():
        if info["count"] > 0:
            lines.append(f"| {path_name} | {info['count']} |")
    ...
    lines.extend([
        ...
        "---",
        "",
        "Submission PDFs are now in a flat folder for easy distribution.",
    ])
...
Shared Content (.shared/ self-contained core):
  ...
  py 3_package_review.py --sync-shared     # Sync ONLY .shared/ (no packages)
  py 3_package_review.py --all --no-sync-shared  # Skip auto-sync
...
Output: packages/pdf/arxiv/, packages/pdf/journal/, etc. (~14 MB total)
    ...
    parser.add_argument("--submission-pdfs", action="store_true",
                        help="Extract submission PDFs to flat folder: packages/pdf/submission_pdfs/")
    ...
    # Handle --submission-pdfs mode
    if args.submission_pdfs:
        result = extract_submission_pdfs(dry_run=args.dry_run)
        if args.json:
            print(json.dumps(result, indent=2))
        else:
            print(generate_submission_pdf_report(result))
        return
    ...
    # AUTO-EXTRACT submission PDFs as final step (when using --all)
    submission_pdf_result = None
    if args.all or "all" in args.paths:
        print()
        print("=" * 60)
        print("STEP 3: Extracting submission PDFs to flat folder")
        print("=" * 60)
        submission_pdf_result = extract_submission_pdfs(dry_run=args.dry_run)
        print(generate_submission_pdf_report(submission_pdf_result))
    ...
    if args.json:
        combined = {
            ...
            "packages": results,
            "submission_pdfs": submission_pdf_result
        }
