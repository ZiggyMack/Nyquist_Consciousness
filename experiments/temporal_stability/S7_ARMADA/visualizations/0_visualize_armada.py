...
===================================================
MASTER orchestrator that generates ALL visualizations for IRON CLAD data.

DEFAULT BEHAVIOR:
  python 0_visualize_armada.py                # Generate ALL visualizations
                                              # (core + subdirectory generators)

USAGE:
  python 0_visualize_armada.py                # Auto-detect + run all generators
  python 0_visualize_armada.py --with-pdfs    # Also generate PDF summaries
  python 0_visualize_armada.py --no-subdirs   # Skip subdirectory generators
  python 0_visualize_armada.py --run 008      # Specific run only
  python 0_visualize_armada.py --list         # List available runs
  python 0_visualize_armada.py --dB           # Use decibel scaling
...
SUBDIRECTORY GENERATORS (called by default):
  ...
  - pics/12_Metrics_Summary/generate_network_improved.py
  - pics/12_Metrics_Summary/generate_exit_survey.py
  - pics/13_Model_Waveforms/generate_model_waveforms.py
...
# =============================================================================
# SUBDIRECTORY GENERATOR ORCHESTRATION
# =============================================================================

def run_subdirectory_generators():
    """Run all visualization generators in pics/ subdirectories.

    IRON CLAD generators that produce final publication-quality outputs:
    - 15_Oobleck_Effect: Prosecutor vs Defense phase dynamics
    - run020: Value/Exchange/Closing analysis
    - Other specialized generators (FFT, PFI, Radar, etc.)
    ...
    - 14_CONSCIOUSNESS/visualize_quartz_rush.py -> 16_Laplace_Analysis (quartz_*.png)
    ...
    root_generators = [
        ...
        (BASE_DIR / "14_CONSCIOUSNESS" / "visualize_quartz_rush.py", BASE_DIR / "14_CONSCIOUSNESS"),  # -> 16_Laplace_Analysis (quartz_*.png)
    ...
    pics_generators = [
        # IRON CLAD primary generators
        OUTPUT_DIR / "15_Oobleck_Effect" / "generate_oobleck_effect.py",
        OUTPUT_DIR / "run020" / "generate_run020_visualizations.py",
        ...
        OUTPUT_DIR / "12_Metrics_Summary" / "generate_network_improved.py",
        OUTPUT_DIR / "12_Metrics_Summary" / "generate_exit_survey.py",
        OUTPUT_DIR / "13_Model_Waveforms" / "generate_model_waveforms.py",
    ]

    print("\n" + "=" * 70)
    print("RUNNING SUBDIRECTORY GENERATORS")
    print("=" * 70)

    success_count = 0
    total_count = len(root_generators) + len(pics_generators)

    # Run root-level generators first
    print("\n  [Root-level generators]")
    for gen_path, cwd in root_generators:
        if gen_path.exists():
            print(f"\n  Running: {gen_path.name}")
            try:
            ...
            print(f"\n  Skipped (not found): {gen_path.name}")

    # Run pics/ subdirectory generators
    print("\n  [pics/ subdirectory generators]")
    for gen_path in pics_generators:
        if gen_path.exists():
    ...
    print(f"\nSubdirectory generators: {success_count}/{total_count} completed")
...
def run_pdf_generators():
    ...
    pdf_generators = [
        ...
        OUTPUT_DIR / "run018" / "generate_pdf_summary.py",
        OUTPUT_DIR / "run020" / "generate_pdf_summary.py",
    ]

    print("\n" + "=" * 70)
    print("RUNNING PDF GENERATORS")
    print("=" * 70)
    ...
    print(f"\nPDF generators: {success_count}/{len(pdf_generators)} completed")
    return success_count
...
def main():
    ...
    parser.add_argument('--no-subdirs', action='store_true',
                       help='Skip subdirectory generators (faster for core viz only)')
    parser.add_argument('--with-pdfs', action='store_true',
                       help='Also generate PDF summaries after visualizations')
    ...
    # Run subdirectory generators (unless --no-subdirs or specific --run/--type)
