...
def load_run_data(run_id: str) -> list:
    """Load all data files for a run, preferring manifest trajectories."""
    results = []

    # Try manifest first - look for trajectories
    manifest_patterns = [
        ...
        if manifest_path.exists():
            try:
                with open(manifest_path, 'r', encoding='utf-8') as f:
                    ...
                    # Manifest has trajectories list with drifts
                    if 'trajectories' in manifest:
                        results.append({'subjects': manifest['trajectories']})
                        print(f"  Loaded {len(manifest['trajectories'])} trajectories from {pattern}")
            except (json.JSONDecodeError, FileNotFoundError) as e:
                print(f"  Error loading {pattern}: {e}")

    # Fallback: scan individual consolidated files
    if not results:
        print("  No manifest trajectories found, scanning individual files...")
        data_files = list(RESULTS_DIR.glob(f"*{run_id}*.json"))
        print(f"  Found {len(data_files)} files")

        for filepath in data_files[:10]:  # Sample first 10
            try:
...
def extract_provider_metrics(run_data: list) -> dict:
    ...
    for data in run_data:
        ...
        # Handle results format (older format)
...
def main():
    ...
    parser.add_argument('--type', choices=['provider', 'pillar', 'pfi'], default='provider',
