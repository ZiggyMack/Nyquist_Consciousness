#!/usr/bin/env python3
"""
13_Model_Waveforms: PDF Summary Generator
==========================================
Creates a PDF summary with all model waveform visualizations embedded.

Reads: 13_model_waveforms_explained.md
Outputs: 13_Model_Waveforms_Summary.pdf

Images embedded in order:
1. fleet_waveform_comparison.png - All 25 models overlaid
2. waveforms_major_providers.png - 2x2 QUAD provider overlays
3. waveforms_together_models.png - 2x2 QUAD family overlays
4-9. Individual model waveforms (top 6 by sample size)
"""
...
SECTION_IMAGES = {
    "1. fleet_waveform_comparison.png": "fleet_waveform_comparison.png",
    "2. waveforms_major_providers.png": "waveforms_major_providers.png",
    "3. waveforms_together_models.png": "waveforms_together_models.png",
    "4-9. Individual Model Waveforms": [
        "waveform_claude-3-5-haiku-20241022.png",
        "waveform_gpt-5-nano.png",
        "waveform_gpt-4.1-nano.png",
        "waveform_gemini-2.5-flash-lite.png",
        "waveform_gemini-2.0-flash.png",
        "waveform_grok-4-1-fast-reasoning.png",
    ],
}
...
def create_pdf(input_md: Path, output_pdf: Path):
    ...
    for elem_type, content in elements:
        if elem_type == 'h1':
            ...
            if current_h3 and paragraphs_since_h3 == 3:
                for section_key, img_file in SECTION_IMAGES.items():
                    if section_key in current_h3:
                        if isinstance(img_file, list):
                            # Multiple images (individual models)
                            for f in img_file:
                                img_path = base_dir / f
                                if embed_image(story, img_path, large=False):
                                    images_embedded += 1
                        else:
                            # Single image
                            img_path = base_dir / img_file
                            is_large = 'fleet' in img_file or 'providers' in img_file or 'together' in img_file
...
def main():
    ...
    input_file = script_dir / '13_model_waveforms_explained.md'
    output_file = script_dir / '13_Model_Waveforms_Summary.pdf'
