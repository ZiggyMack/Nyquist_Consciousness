...
IRON CLAD PDF SUMMARY MASTER
============================
DEFAULT BEHAVIOR:
  python 1_generate_pdf_summaries.py    # Generate ALL PDFs
...
2. Subdirectory PDF generators (automatically called):
   ...
   - pics/run018/generate_pdf_summary.py
   - pics/run020/generate_pdf_summary.py
...
See also:
    - 0_visualize_armada.py       -> MASTER for image generation
    - 2_RnD_Visualization.py      -> R&D wing for experimental prototypes
...
def generate_metrics_summary_pdf():
    ...
    story.append(Paragraph(
        ...
        "from 825 experiments across 51 models and 5 providers.",
        body_style
    ))
    story.append(Paragraph(
    ...
    add_image(story, img_path, width=6.5*inch, caption="Figure 1: Full Fleet Network - 51 models across 5 providers")

    story.append(Paragraph(
        "<b>What it shows:</b> Network graph of the entire ARMADA fleet organized by provider. "
        "Each hexagonal hub represents a provider (Anthropic, OpenAI, Google, xAI, Together.ai). "
    ...
    story.append(Paragraph(
        ...
        "representation. Anthropic (7), xAI (9), and Google (5) complete the fleet. NVIDIA's Nemotron is accessed via Together.ai.",
        body_style
    ))
    story.append(PageBreak())
    ...
    # ========== SECTION 7: MANIFOLD EDGE DETECTION - MAJOR PROVIDERS ==========
    story.append(Paragraph("7. Manifold Edge Detection - Major Providers", heading_style))
    img_path = PICS_DIR / "12_Metrics_Summary" / "manifold_edge_major_providers.png"
    add_image(story, img_path, width=6.5*inch, caption="Figure 7a: Manifold edge dynamics for major providers (Anthropic, OpenAI, Google, xAI)")

    story.append(Paragraph(
        "<b>What it shows:</b> Visualization of the identity manifold's edges - the boundaries "
        "where identity becomes unstable. This analysis identifies models from the major commercial "
        "providers that operate near the edge vs. those safely in the interior of identity space.",
        body_style
    ))
    story.append(Paragraph(
        "<b>Key concepts:</b><br/>"
    ...
    # ========== SECTION 7b: MANIFOLD EDGE DETECTION - TOGETHER.AI MODELS ==========
    story.append(Paragraph("7b. Manifold Edge Detection - Together.ai Models", heading_style))
    img_path = PICS_DIR / "12_Metrics_Summary" / "manifold_edge_together_models.png"
    add_image(story, img_path, width=6.5*inch, caption="Figure 7b: Manifold edge dynamics for Together.ai open-source models")

    story.append(Paragraph(
        "<b>What it shows:</b> The same manifold edge analysis applied to open-source models hosted "
        "by Together.ai, including Meta/Llama, DeepSeek, Mistral, Qwen, and other model families. "
        "These models show more diverse behaviors due to varied training approaches.",
        body_style
    ))
    story.append(Paragraph(
        "<b>Provider note:</b> Together.ai hosts multiple model families (Llama, Qwen, Mistral, etc.) "
        "including NVIDIA's Nemotron models. All models accessed via Together.ai are counted under "
        "the 'Together.ai' provider category, giving 5 total providers: Anthropic, OpenAI, Google, xAI, Together.ai.",
        body_style
    ))
    story.append(PageBreak())
    ...
    story.append(Paragraph(
        ...
        "51 models across 5 providers: Anthropic (7), OpenAI (14), Google (5), xAI (9), Together.ai (17 including NVIDIA Nemotron).",
        body_style
    ))
    story.append(Paragraph(
...
def run_subdirectory_pdf_generators():
    ...
    These generators handle run-specific PDF summaries that are not
    covered by the main PDF generation functions above.
    """
    import sys
    import subprocess

    pdf_generators = [
        PICS_DIR / "10_PFI_Dimensional" / "md_to_pdf.py",
        PICS_DIR / "13_Model_Waveforms" / "generate_pdf_summary.py",
        PICS_DIR / "15_Oobleck_Effect" / "generate_pdf_summary.py",
        PICS_DIR / "run018" / "generate_pdf_summary.py",
        PICS_DIR / "run020" / "generate_pdf_summary.py",
    ]

    print("\n" + "=" * 60)
    print("RUNNING SUBDIRECTORY PDF GENERATORS")
    print("=" * 60)

    success_count = 0
    for gen_path in pdf_generators:
        if gen_path.exists():
            print(f"\n  Running: {gen_path.parent.name}/{gen_path.name}")
            try:
            ...
            print(f"\n  Skipped (not found): {gen_path.parent.name}/{gen_path.name}")

    print(f"\nSubdirectory PDF generators: {success_count}/{len(pdf_generators)} completed")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("ALL PDF SUMMARIES COMPLETE")
    print("=" * 60)
