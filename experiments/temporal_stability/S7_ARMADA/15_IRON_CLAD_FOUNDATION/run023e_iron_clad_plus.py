"""
S7 RUN 023e: IRON CLAD PLUS - FULL ARMADA CONTROLLABILITY
===========================================================
Tests remaining 27 models NOT in budget_patrol-lite at N=3 iterations.

PURPOSE:
    Run 023d tested 25 budget_patrol-lite ships and found:
    - OpenAI nano/mini models: 0% controllable
    - Claude/Llama lite models: ~80% controllable

    But this only tested LITE/DISTILLED models!

    Key question: Do FULL-CAPABILITY models show controllability?
    If distillation strips controllability, the frontier models should retain it.

    This run tests:
    - Yacht tier: claude-opus-4.5, o3, grok-4, etc.
    - High-maintenance tier: claude-sonnet-4.5, gpt-4.1, gpt-4o, etc.
    - Armada tier: gpt-5.1, gpt-5, o3-mini, o4-mini, llama3.1-405b, etc.
    - Remaining patrol/budget: llama3.1-70b, gpt-3.5-turbo, etc.

PROTOCOL:
    Same as Run 023d (extended settling + Oobleck controllability):
    - Phase 1: BASELINE (3 probes)
    - Phase 2: STEP INPUT (1 probe)
    - Phase 3: RECOVERY (up to 20 probes until settled)
    - Phase 4: CONTROL DEMO (if timeout) - DRIVE_UP + DRIVE_DOWN

FLEET:
    27 untested models x N=3 = 81 experiments
    Estimated cost: ~$13 (mostly from yacht tier)

USAGE:
    # Pilot run (1 ship, 1 iteration)
    python run023e_iron_clad_plus.py --pilot

    # Full run (27 ships, 3 iterations)
    python run023e_iron_clad_plus.py

    # Dry run (test flow without API calls)
    python run023e_iron_clad_plus.py --dry-run

    # Override iterations
    python run023e_iron_clad_plus.py -N 5

Author: Claude (VALIS Network)
Date: December 21, 2025
IRON CLAD Layer: 4 (IRON CLAD+ - Full Armada Controllability)
"""
...
def load_raw_manifest() -> Dict:
    """Load the raw ARCHITECTURE_MATRIX.json with all fields including tier, cost, etc."""
    manifest_path = ARMADA_DIR / "0_results" / "manifests" / "ARCHITECTURE_MATRIX.json"
    if not manifest_path.exists():
        raise FileNotFoundError(f"ARCHITECTURE_MATRIX.json not found at {manifest_path}")
...
# =============================================================================
# IRON CLAD+ FLEET DEFINITION
# =============================================================================

def get_iron_clad_plus_fleet() -> Tuple[List[str], Dict[str, Dict]]:
    """
    Get the 27 models NOT tested in Run 023d (budget_patrol-lite).

    Returns:
        Tuple of (list of ship names, dict of ship_name -> full config with tier info)
    """

    # Models tested in 023d (by model ID, not ship name)
    TESTED_IN_023D = {
    ...
    # Legacy provider aliases to skip
    LEGACY_ALIASES = {"anthropic", "openai", "google", "xai", "together", "deepseek"}

    # Load raw manifest to get tier info
    manifest = load_raw_manifest()
    ships = manifest.get("ships", {})

    # Find ships whose model IDs are NOT in the tested set
    untested = []
    full_configs = {}
    for ship_name, info in ships.items():
        # Skip legacy provider aliases
        if ship_name in LEGACY_ALIASES:
            continue

        model_id = info.get("model", ship_name)
        if model_id not in TESTED_IN_023D:
            untested.append(ship_name)
            full_configs[ship_name] = info

    return sorted(untested), full_configs
...
class ExtendedSettlingResult:
    experiment: str = "iron_clad_plus"
    model: str = ""
    provider: str = ""
    tier: str = ""  # NEW: Track tier for analysis
    iteration: int = 0
...
def update_status_summary(data: Dict, results_file: Path):
    """Update STATUS_SUMMARY_023e.txt with human-readable progress."""

    status_file = RESULTS_DIR / "STATUS_SUMMARY_023e.txt"

    results = data.get("results", [])
    total_results = len(results)
    target = data.get("target", 81)
    ...
    # Gaps analysis
    gaps = target - total_results

    lines = [
        "=" * 70,
        "RUN 023e: IRON CLAD PLUS - FULL ARMADA CONTROLLABILITY",
        "=" * 70,
        f"Updated: {datetime.now().isoformat()}",
        f"Results file: {results_file.name}",
        "",
        f"FLEET: IRON CLAD+ (27 untested models) x N=3 = 81 target",
        f"TOTAL RESULTS: {total_results} / {target} ({100*total_results/target:.0f}% complete)",
        f"GAPS REMAINING: {gaps}",
        "",
        "STATUS: " + ("COMPLETE" if gaps == 0 else "IN PROGRESS"),
        "",
        "STABILITY CLASSIFICATION:",
        f"  STABLE:        {stable:4d} ({100*stable/total_results:.0f}%)" if total_results else "  STABLE:        0",
        f"  VOLATILE:      {volatile:4d} ({100*volatile/total_results:.0f}%)" if total_results else "  VOLATILE:      0",
        f"  CONTROLLABLE:  {controllable:4d} ({100*controllable/total_results:.0f}%)" if total_results else "  CONTROLLABLE:  0",
        "",
        "BY TIER:",
    ]

    tier_order = ["yacht", "high_maintenance", "armada", "patrol", "budget"]
    ...
    lines.extend([
        "",
        "SETTLING BEHAVIOR:",
        f"  Naturally settled:  {naturally_settled:4d} ({100*naturally_settled/total_results:.0f}%)" if total_results else "  Naturally settled: 0",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run 023e: IRON CLAD+ Full Armada Controllability",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Purpose:
  Test the 27 models NOT in budget_patrol-lite to complete controllability
  testing across the full armada. Key question: Do frontier models retain
  controllability that distillation strips from lite models?
...
Examples:
  python run023e_iron_clad_plus.py --pilot
  python run023e_iron_clad_plus.py -N 3
  python run023e_iron_clad_plus.py --dry-run
"""
    )
    ...
    parser.add_argument("--pilot-ship", default="claude-sonnet-4.5",
                       help="Ship for pilot run (default: claude-sonnet-4.5)")
    parser.add_argument("-N", "--iterations", type=int, default=3,
                       help="Number of iterations per ship (default: 3)")
    ...
    parser.add_argument("--skip-yacht", action="store_true",
                       help="Skip expensive yacht tier models")
    ...
    # Get fleet - load raw manifest for full config with tier info
    architecture_matrix = load_architecture_matrix()  # Basic config for API calls
    ships_list, full_configs = get_iron_clad_plus_fleet()  # Full configs with tier
    ...
    if args.pilot:
        ...
        if args.skip_yacht:
            ships = [s for s in ships if full_configs.get(s, {}).get("tier") != "yacht"]
            print(f"[INFO] Skipping yacht tier (--skip-yacht)")

    print(f"\n{'=' * 70}")
    print("RUN 023e: IRON CLAD PLUS - FULL ARMADA CONTROLLABILITY")
    print(f"{'=' * 70}")
    print(f"Fleet: IRON CLAD+ ({len(ships)} ships)")
    ...
    tier_counts = {}
    for s in ships:
        tier = full_configs.get(s, {}).get("tier", "unknown")
        tier_counts[tier] = tier_counts.get(tier, 0) + 1
    ...
    results_file = RESULTS_DIR / f"{prefix}S7_run_023e_CURRENT.json"
    checkpoint_file = RESULTS_DIR / f"_CHECKPOINT_run023e_{run_timestamp}.json"
    ...
    def save_data():
        ...
        data = {
            "run_id": "023e",
            "run_name": "IRON CLAD+ Full Armada Controllability",
            "timestamp": run_timestamp,
            "target": len(ships) * iterations,
    ...
    for ship_name in ships:
        ...
        # Add tier and provider from full_configs
        if ship_name in full_configs:
            model_config["tier"] = full_configs[ship_name].get("tier", "unknown")
            model_config["provider"] = full_configs[ship_name].get("provider", "unknown")
