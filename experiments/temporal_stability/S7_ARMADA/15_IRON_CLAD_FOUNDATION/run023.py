"""
S7 RUN 023: IRON CLAD FOUNDATION (Cosine Distance, N=30)
=========================================================
...
DATA OUTPUT:
    results/S7_run_023_CURRENT.json

RELATED SCRIPTS:
    run023.py (this script)     - Foundation experiments (6 types, L1-L3)
    run023_extended.py          - Extended settling + Oobleck control demo
    visualize_023.py            - Unified visualization generator
    utility/fill_gaps.py        - Fill missing experiments
    utility/monitor_progress.py - Monitor long-running experiments
...
METHODOLOGY:
    ...
    Event Horizon = 0.80 (calibrated from run023 data)
    Warning = 0.60
    Catastrophic = 1.20
...
EXPERIMENTS:
    1. event_horizon - Validate 0.80 threshold with perturbation sequence
    2. stability - STABLE vs VOLATILE classification across fleet
    3. recursive - Cognitive waveform capture with recursive loops
    4. boundary - Boundary zone mapping (0.60/0.80/1.20 thresholds)
...
USAGE:
    ...
    python run023.py --iterations 30 --providers budget_patrol-lite

    # Specific experiment only
    python run023.py --experiment event_horizon

    # Dry run (test flow without API calls)
    python run023.py --dry-run
