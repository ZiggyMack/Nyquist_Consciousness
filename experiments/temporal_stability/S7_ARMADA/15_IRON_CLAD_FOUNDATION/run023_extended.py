"""
S7 RUN 023 EXTENDED: SETTLING TIME + OOBLECK CONTROLLABILITY
=============================================================
Collects 20-probe settling data with Oobleck controllability demonstration.

PURPOSE:
    Run 023 foundation settling experiments had only 6 probes - insufficient for:
...
FLEET OPTIONS:
    --fleet budget_patrol-lite   (25 ships - default)
    --fleet iron_clad_plus       (27 additional untested models)
    --fleet armada-full          (all 52 ships)

USAGE:
    # Pilot run (1 ship, 1 iteration)
    python run023_extended.py --pilot

    # Full run with budget_patrol fleet (25 ships, 30 iterations)
    python run023_extended.py --iterations 30

    # Full armada run (27 additional ships, 3 iterations)
    python run023_extended.py --fleet iron_clad_plus -N 3

    # Dry run (test flow without API calls)
    python run023_extended.py --dry-run

Author: Claude (VALIS Network)
Date: December 20, 2025 (consolidated December 29, 2025)
IRON CLAD Layer: 3-4 (MAPPING - Extended Settling + Full Armada)

CONSOLIDATION NOTE:
    This script unifies run023d (budget_patrol-lite) and run023e (iron_clad_plus)
    into a single script with fleet selection via --fleet flag.
"""
...
# =============================================================================
# IRON CLAD PLUS FLEET (models not in budget_patrol-lite)
# =============================================================================

# Models already tested in budget_patrol-lite fleet
TESTED_IN_BUDGET_PATROL = {
...
def get_iron_clad_plus_fleet() -> Tuple[List[str], Dict[str, Dict]]:
    """
    Get models NOT tested in budget_patrol-lite fleet.
    These are the "iron_clad_plus" fleet for extended testing.
    """
    ...
    for ship_name, info in manifest.items():
        if ship_name in LEGACY_ALIASES:
            continue

        model_id = info.get("model", "")
        if model_id not in TESTED_IN_BUDGET_PATROL:
...
def update_status_summary(data: Dict, results_file: Path):
    """Update STATUS_SUMMARY_023_extended.txt with human-readable progress."""

    status_file = RESULTS_DIR / "STATUS_SUMMARY_023_extended.txt"
    ...
    lines = [
        "=" * 70,
        "RUN 023 EXTENDED: SETTLING + OOBLECK CONTROLLABILITY",
        "=" * 70,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run 023 Extended: Settling Time + Oobleck Controllability",
...
Fleet Options:
  --fleet budget_patrol-lite   25 budget/patrol tier ships (default)
  --fleet iron_clad_plus       27 untested models (yacht/armada/etc)
...
"""
    ...
    parser.add_argument("--fleet", default="budget_patrol-lite",
                       help="Fleet selection: budget_patrol-lite (default), iron_clad_plus, or any fleet_loader option")
    ...
    parser.add_argument("--skip-yacht", action="store_true",
                       help="Skip yacht tier models (iron_clad_plus only)")
    ...
    # Get fleet and tier info
    tier_info = {}  # ship_name -> tier
    ...
    elif args.fleet == "iron_clad_plus":
        # Special fleet: models not in budget_patrol-lite
        ships, full_configs = get_iron_clad_plus_fleet()

        # Extract tier info
        for ship_name, config in full_configs.items():
            tier_info[ship_name] = config.get("tier", "unknown")

        # Apply skip-yacht filter
        if args.skip_yacht:
            original_count = len(ships)
            ships = [s for s in ships if tier_info.get(s) != "yacht"]
            print(f"[INFO] Skipping yacht tier: {original_count} -> {len(ships)} ships")

        iterations = args.iterations
    else:
        # Standard fleet from fleet_loader
        try:
            ships = get_fleet_by_option(args.fleet)
    ...
    print(f"\n{'=' * 70}")
    print("RUN 023 EXTENDED: SETTLING + OOBLECK CONTROLLABILITY")
    print(f"{'=' * 70}")
    print(f"Fleet: {args.fleet if not args.pilot else 'PILOT'} ({len(ships)} ships)")
    ...
    results_file = RESULTS_DIR / f"{prefix}S7_run_023_extended_CURRENT.json"
    checkpoint_file = RESULTS_DIR / f"_CHECKPOINT_run023_extended_{run_timestamp}.json"
    ...
    for ship_name in ships:
        ...
        for iteration in range(iterations):
            ...
            try:
                ...
                result = run_extended_settling(ship_name, model_config, iteration, tier=tier)
